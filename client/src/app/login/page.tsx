...
export default function Login() {
  ...
  useEffect(() => {
    ...
    else if (user.role === 'university') router.push('/university/dashboard');
    else if (user.role === 'freelancer') router.push('/freelancer/dashboard');
    else toast.error(`Unknown user role: ${user.role}`);
  }, [user, router]);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#102c24] via-[#1d3d33] to-[#2e4e49] p-4 relative overflow-hidden">
      {/* Animated Background Elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-20 left-10 w-72 h-72 bg-[var(--brand-primary-lighter)]/20 rounded-full opacity-20 blur-3xl animate-pulse-slow"></div>
        <div className="absolute bottom-20 right-10 w-96 h-96 bg-[var(--brand-primary-light)]/20 rounded-full opacity-20 blur-3xl animate-pulse-slow" style={{ animationDelay: '1s' }}></div>
