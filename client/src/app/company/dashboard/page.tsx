    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const [profileResponse, statsResponse, activityResponse] = await Promise.all([
          fetch(`${API_URL}/company/profile`, { headers: authHeaders }),
          fetch(`${API_URL}/company/dashboard/stats`, { headers: authHeaders }),
          fetch(`${API_URL}/company/dashboard/activity`, { headers: authHeaders })
        ]);
        ...
        if (profileResponse.ok) {
          ...
          if (profileData.success && profileData.data) {
            setCompanyProfile({
              companyName: profileData.data.companyName || 'Company',
              industry: profileData.data.industry || 'Not specified',
              verificationStatus: profileData.data.verificationStatus || 'pending'
            });
          } else {
            console.warn('Profile data not available:', profileData.message);
          }
        } else {
        ...
        if (statsResponse.ok) {
          ...
          if (statsData.success && statsData.data) {
            setStats({
              ...
              feedbackGiven: statsData.data.feedbackGiven || 0
            });
          } else {
            console.warn('Stats data not available:', statsData.message);
          }
        } else {
        ...
        if (activityResponse.ok) {
          ...
          if (activityData.success && activityData.data) {
            const formattedActivity = activityData.data.map((item: any) => ({
              ...
              time: item.time ? new Date(item.time).toLocaleString() : new Date().toLocaleString()
            }));
            setRecentActivity(formattedActivity);
          } else {
            console.warn('Activity data not available:', activityData.message);
          }
        } else {
          console.warn('Failed to fetch activity:', activityResponse.status);
        }
                  ...
                  <div className="ml-3">
                    ...
                    <div className="mt-4">
                      <button
                        ...
                        className="bg-red-100 px-3 py-2 rounded-md text-sm font-medium text-red-800 hover:bg-red-200 transition"
                      >
                        Retry
                      </button>
                    </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-indigo-500">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-800">{stats.feedbackGiven}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          >
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <h3 className="font-semibold text-gray-800">View Applicants</h3>
          ...
          >
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-sm text-gray-600">Provide and view feedback</p>
              </div>
            </div>
          </Link>
          ...
          >
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-sm text-gray-600">View all job postings</p>
              </div>
            </div>
          </Link>
        </div>
          ...
          {recentActivity.length > 0 ? (
            <div className="space-y-4">
              {recentActivity.map(activity => (
                <div key={activity.id} className="flex items-center space-x-4 p-3 hover:bg-gray-50 rounded-lg transition">
