      ...
      <main className="relative mx-auto max-w-7xl px-6 py-8">
        ...
        {error && (
          <div className="mb-8 rounded-xl border border-red-200 bg-red-50 p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-medium text-red-800">Error loading quizzes</div>
        ...
        {!loading && !error && quizzes.length === 0 && (
          <div className="rounded-2xl border border-dashed border-[#C8D9E6] bg-white/60 p-12 text-center">
            ...
            <p className="text-sm text-[#567C8D] mb-4">Check back later for new skills assessments!</p>
            <button 
              onClick={fetchQuizzes}
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="mt-8 rounded-lg bg-gray-100 p-4 text-sm">
            ...
            <div className="space-y-1 text-gray-600">
              ...
              <div>Quizzes count: {quizzes.length}</div>
              <div>User role: {user?.role}</div>
              <div>API Base: {process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}</div>
            </div>
          </div>
        )}
      </main>
